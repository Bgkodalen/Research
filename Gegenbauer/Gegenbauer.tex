...
we see that $q_{d+1}(\vert X\vert E_1) = 0$ and therefore every polynomial function $F$ may be written as a linear combination of the polynomials $q_0,q_1,\dots,q_d$ so that $F(\vert X\vert E_1) = \sum_j f_j q_j(\vert X\vert E_1)$ when applied entrywise.
***Major confusion here... this part is not done***
