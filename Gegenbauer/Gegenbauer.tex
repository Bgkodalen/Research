...
\begin{proof}
	...
	1 & s & -2(s+1) & s & 1\\
	\end{array}\right].\]
	The final requirement that $s = -n^2$ comes from the structure of $L_1^*$. For a $Q$-bipartite scheme, the last row of $L_1^*$ must be $\ell = \left[0,0,\dots,m,0\right]$. However, since column $i$ of $P$ must be an eigenvector of $L_1^*$ with eigenvalue $Q_{i,1}$, we consider column 1 of $P$ giving us $-nm = \frac{m}{n}s$ or $s = -n^2$ as desired.
\end{proof}
\begin{corollary}
...
\end{theorem}
\begin{proof}
	For $(i)$ and $(ii)$, we note that	$p_{13}^1 = \frac{(n-1)(\mu-n(r+n))}{2n}$. Since this must be a non-negative integer and $s\geq n$, we must either have $-s = n = 1$ or $\mu-n(r+n)>0$. As $s=-1$ implies our SRG is a union of cliques, we may ignore this case and $(i)$ follows. Further, since $n\not\vert (n-1)$, we have that $n\vert (\mu-n(r+n))$ forcing $n\vert \mu$. Since $k=\mu+rn^2$, $(ii)$ follows. $(iii)$ follows from $(i)$ as $\mu = k-rn^2$. Finally, $(iv)$ follows from the absolute bound: $1+f \leq \frac{m(m+1)}{2}$ giving us $n^4+3n^2r-2k\geq 0$. Note that $q_{22}^2 = \frac{(kn^2+r^2)(n^2-1)(n^2+k)}{(n^2+r)^2\mu}-\frac{(n^2+k)}{(n^2+r)}$, so that we must have $(kn^2+r^2)(n^2-1)\geq (n^2+r)\mu$. However since $n^2>1$, this is weaker than $r^2(n^2-1)>rk-n^4r-n^2r^2$ and since $k\leq (3rn^2+n^4)/2$ we must have 
