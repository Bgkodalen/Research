...
\begin{frame}
	...
	\begin{tikzpicture}
		\def\x{0}
		\def\y{0}
		\node at (\x,\y) (classes) {d};
		\node at (\x+1,\y) (top) {};
		\node at (\x+1,\y-2) (bot) {};
		\node at (\x,\y-1) (left) {};
		\node at (\x+2,\y-1) (right) {};
		\draw[-] top -- bot;
		\draw[-] left -- right;
		\node at (\x+.5,\y-.5) (prim) {primitive};
		\node at (\x+1.5,\y-.5) (bip) {$Q$-bipartite};
		\node at (\x+.5,\y-1.5) (ant) {$Q$-antipodal};
		\node at (\x+1.5,\y-1.5) (both) {both};
	\end{tikzpicture}
\end{frame}
