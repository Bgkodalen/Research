...
\begin{frame}
	...
	\begin{tikzpicture}
		...
		\only<3>{
			...
			\node[align=center] at (\x+3*\s,\y-\s) (bip) {Lines with two\\IPs: $\{0,\pm\alpha\}$\\\\Double covers of SRGs};
		...
		\only<4>{
			...
			\node[align=center] at (\x+3*\s,\y-\s) (bip) {Lines with two\\IPs: $\{\pm \alpha,\pm\beta\}$\\\\Double covers of SRGs};
			\node[align=center] at (\x+\s,\y-3*\s) (ant) {``\textit{LSSRD}"\\$[Thm]$\\(Higman `95)};
...
\end{itemize}
\vfill
Simplices $A$ and $B$ are ``linked" if there exists $-1\leq\gamma,\zeta<1$ with:
\[\forall (a,b)\in A\times B: \left<a,b\right> \in\left\{\gamma,\zeta\right\}\]
\vfill
...
\begin{frame}
	...
	A function $f:[-1,1]\rightarrow \mathbb{R}$ is \textit{positive definite} if, for every finite subset $X$, $f\circ\left(G_X\right)\succeq 0$, where $G_X$ is the Gram matrix of $X$.
...
\begin{frame}
	...
	\[\begin{aligned}
	G_0^{(m)} &= 1 \qquad G_1^{(m)} = t\\
	G_{k}^{(m)} &= \frac{(2k+m-4)tG_{k-1}^{(m)}(t)-(k-1)G_{k-2}^{(m)}(t)}{k+m-3}
	\end{aligned}\]\vfill
