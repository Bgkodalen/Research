...
\begin{frame}
	...
	\begin{tikzpicture}
		...
		\only<1>{
			...
			\node[align=center] at (\x+3*\s,\y-1.2*\s) (bip) {Inner products of \\ $\left\{-1,\pm \alpha,\dots\right\}$\\\\``lines through the\\origin"};
			\node[align=center] at (\x+\s,\y-3*\s) (ant) {vectors\\ partitioned into sets \\ of equal size};
		...
		\only<2>{
				...
				\node[align=center] at (\x+\s,\y-3*\s) (ant) {``\textit{LSSD}"\\\\(Cameron `72)};
				\node[align=center] at (\x+3*\s,\y-3*\s) (both) {None exist};%Trivial $LSSD$\\\\ uses $(v,1,0)$ as\\the symmetric design};
		}
		\only<3>{
			\node[align=center] at (\x+\s,\y-\s) (prim) {Spherical\\$4$-distance set};
			\node[align=center] at (\x+3*\s,\y-\s) (bip) {Lines with two\\angles: $\{0,\pm\alpha\}$\\\\Double covers of SRGs};
			\node[align=center] at (\x+\s,\y-3*\s) (ant) {``\textit{LSSSD}"\\\\(Higman `95)};
			\node[align=center] at (\x+3*\s,\y-3*\s) (both) {Mutually Unbiased \\ Bases\\\\(DGS `75)};
		}
		\only<4>{
			\node[align=center] at (\x+\s,\y-\s) (prim) {Spherical\\$4$-distance set};
			\node[align=center] at (\x+3*\s,\y-\s) (bip) {Lines with two\\angles: $\{\pm \alpha,\pm\beta\}$\\\\Double covers of SRGs};
			\node[align=center] at (\x+\s,\y-3*\s) (ant) {``\textit{LSSSSD}"?\\(unknown)};
			\node[align=center] at (\x+3*\s,\y-3*\s) (both) {Mutually unbiased \\ equiangular lines};
		}
	\end{tikzpicture}
\end{frame}
