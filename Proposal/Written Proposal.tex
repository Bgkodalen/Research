...
\begin{document}
	...
	\end{equation}
	We call $P$ and $Q$ the {\em first and second eigenmatrices}, respectively and note here that $P_{0i}$ is the valency of relation $R_i$ and $Q_{0j}$ is the rank of $E_j$. Finally, as our matrix algebra is closed under Schur products, we find that there exist structure constants $q_{i,j}^k$ such that for all $0\leq i,j\leq d$:
	\[E_i\circ E_j = \frac{1}{\vert X\vert}\sum_k q_{i,j}^k E_k.\]
	We call these parameters the Krein parameters of the association scheme. Finally, we call an association scheme \emph{primitive} \cite[Sec.~2.4]{BCN} if the graph $\Gamma(X,R_i)$ is connected for all $i=1,\ldots, d$ and \emph{imprimitive} otherwise.
	...
	We may do much of the same analysis using the second basis of our Bose-Mesner algebra. Similar to before, we call an association scheme \textit{$Q$-polynomial} (\textit{cometric}) whenever the set $\left\{E_0,E_1,\dots,E_d\right\}$ may be ordered so that $q^{k}_{i,j} = 0$ when $k>i+j$ or $k<\vert i- j\vert$ and $q^{k}_{i,j}>0$ whenever $k = i+j$. We call such an ordering or the eigenspaces a \textit{$Q$-polynomial ordering}. Just as with metric schemes, we may define polynomials $q_0, q_1,\dots,q_d$ given by $q_0(t) = 1$, $q_1(t) = t$ and 
	\[q^{i-1}_{i,1}q_i(t) = \left(t-q^{i-1}_{i-1,1}\right)q_{i-1}(t)-q^{i-1}_{i-2,1}q_{i-2}(t)\]
	so that $q_i\circ\left(\vert X\vert E_1\right) = \vert X \vert E_i$ where products here are evaluated elementwise. Additionally, we define two more terms: {\em $Q$-antipodal} and {\em $Q$-bipartite}. Let $(X,\mathcal{R})$ be a $Q$-polynomial association scheme with ordering $E_0,\dots,E_d$. Then $(X,\mathcal{R})$ is \textit{$Q$-antipodal} if $q^{k}_{d,d} >0$ when $k = 0$ but $q^{k}_{d,d} = 0$ for $0<k<d$. Alternatively, $(X,\mathcal{R})$ is \textit{$Q$-bipartite} if $q^k_{i,j}=0$ whenever $k+i+j$ is odd. In 1998, Suzuki \cite{Suzuki} showed that every imprimitive $Q$-polynomial scheme, apart from polygons, is either $Q$-bipartite or $Q$-antipodal.\par
	$P$-polynomial schemes have been studied extensively (see \cite{BCN}) with many strong results including connectivity, unimodality of the valencies, and parameter restrictions. However, many of these theorems turn to conjectures when considered in the dual space of $Q$-polynomial schemes. For instance, it is still an open conjecture by Bannai and Ito that the multiplicities of a $Q$-polynomial scheme are unimodal. Likewise, the connectivity results we have seen for distance-regular graphs have not yet been extended to cometric schemes. Bannai and Ito also conjectured that every primitive cometric scheme of sufficiently large diameter $d$ is metric as well. This hints that, for large class number, we should expect to see $P$-polynomial and $Q$-polynomial schemes acting the same. However, as we will discuss in some of the results of this dissertation, there are still many interesting $Q$-polynomial schemes with low class number which do not admit a metric ordering of the adjacency matrices.
	\section{Connectivity}
	Brouwer and Mesner \cite{bromes} showed in 1985 that the vertex connectivity of a strongly regular graph $\Gamma$ is equal to its valency and that the only disconnecting sets of minimum size are the neighborhoods $\Gamma(a)$  of its vertices.  (Brouwer \cite{brouwer} mentions that the corresponding result for edge connectivity was established by Plesn\'{\i}k in 1975.) This result on vertex connectivity was extended by Brouwer and Koolen \cite{brouwer-koolen} in 2009 to show that a distance-regular graph of valency at least three has vertex connectivity equal to its valency and that the only disconnecting sets of minimum size are again the neighborhoods $\Gamma(a)$. A number of very difficult questions of this sort remain open.\par
	In \cite{connectivity}, we prove the following theorem: 
	\begin{theorem}[Kodalen, Martin] \label{Tmain}
	...
	\begin{corollary}[Kodalen, Martin]  \label{C2}
	...
	\begin{corollary}[Kodalen, Martin] \label{C1}
		Let $(X,\cR)$ be a commutative association scheme.  Assume the undirected graph 
	...
	\begin{corollary}[Kodalen, Martin] \label{C3}
		Let $(X,\cR)$ be a commutative association scheme. Assume the undirected graph 
	...
	Theorems \ref{2design} and \ref{3fibers} then show that every set of linked simplices gives rise to a linked system of symmetric designs.
	...
	Defining a set of linked simplices as optimistic (pessimistic) if more than half the inner products between distinct simplices are positive (negative), Theorem \ref{const} shows a link between linked simplices and equiangular lines.
	 \begin{theorem}[Kodalen]\label{const}
	...
	\end{theorem}
	Exploiting a link between regular unbiased Hadamard matrices and mutually unbiased bases, we then arrive at
	\begin{theorem}[Kodalen]
		\label{equiv}
	...
	\end{theorem}
	Finally, we may use Theorem \ref{newLSSD} to provide examples of non-trivial LSSDs in which $w$ can be made arbitarily large for $v$ not a power of 2 (see corollary \ref{16t} and \ref{36t}).
	\begin{theorem}[Kodalen]
		\label{newLSSD}
...
\end{proof}
\begin{corollary}[Kodalen]\label{16t}
...
\end{proof}
\begin{corollary}[Kodalen]\label{36t}
...
\end{proof}
These constructions are the first infinite families of non-trivial LSSDs which do not require $v = 4^n$. Every construction from these theorems is an optimistic LSSD and uses Menon parameters $(v,k,\lambda) = (4u^2,2u^2-u,u^2-u)$ for the symmetric designs between fibers. The existence of a single non-trivial LSSD using parameters not of this form is still an open question. It is the belief of this author that no pessimistic LSSD with more than two fibers exists, though no proof of this statement has been found. As a step in that direction, we survey the known infinite families of symmetric designs and show, using basic number theoretic conditions, that only a few families may be used to build LSSDs with more than two fibers. If we find that pessimistic LSSDs do not exist, then only the Menon and Wallis-McFarland parameter sets remain feasible out of the known infinite families. It is worth noting that the intersection of these two families gives us exactly the parameters of the classical examples requiring $v=4^n$.
\section{Sch\"{o}nberg's theorem}
In this section, we begin by outlining some work done by Delsarte, Goethals, and Seidel in \cite{DGS}; original work begins after Theorem \ref{psdgeg}. As this is ongoing work, we will provide more details in the proofs than we have in previous sections. Let $\Omega_m$ denote the unit sphere in Euclidean space $\mathbb{R}^m$. For each finite $X \subset \Omega_m$, let $G_X$ denote the Gram matrix of $X$. This matrix is always positive semidefinite; for Hermitian matrices $M$ and $N$ of the same size, we write $M \succeq N$ to denote that the matrix $M-N$ is positive semidefinite. A function $f:[-1,1]\rightarrow \re$ is \emph{positive definite} if, for every finite subset $X\subset\Omega_m$, $f\circ (G_X) \succeq 0$ where $f \circ (M)$ has entries $f(m_{ij})$ when $M=[m_{ij}]$ is a matrix and $f$ is an function of a single variable. Below, we define a set of positive definite functions which can be used to disprove the existence of certain $Q$-polynomial association schemes.\\
For any $k\geq 0$ let $\text{Harm}(k) = \text{Harm}_m(k)$ denote the space of harmonic and homogeneous polynomials of degree $k$ on $\Omega_m$. Let $N = \dim\text{Harm}(k)$ and fix an orthogonal basis $\left\{W_{k,i}\right\}_{i=1..N}$ of $\text{Harm}(k)$. We define the {\em Gegenbauer polynomials} for dimension $m$ via $Q_0^{(m)}(t) = 1$, $Q_1^{(m)}(t) = t$ and the three term recurrence:
...
However, since $v^TH_kH_k^Tv = \lVert{vH_k^T}\rVert^2\geq 0$ for any vector $v\in \re^{\vert X\vert}$, we must have that $Q_k\circ(G_X)$ is a positive semidefinate Hermitian matrix and thus $Q_k\circ$ is a positive definite function for any choice of $k$.
\begin{corollary}[Kodalen]\label{nonneg}
...
\begin{theorem}[Kodalen] \label{generalschoen}Let $(X,\cR)$ be a symmetric $Q$-polynomial association scheme with natural ordering $E_0,E_1,\dots,E_d$. Then the following inequalities must be satisfied.
\begin{itemize}
...
\begin{theorem}[Kodalen]\label{4bip}
	Let $(X,\mathcal{R})$ be a 4-class $Q$-bipartite association scheme with $Q$-polynomial ordering $E_0,E_1,\dots,E_d$. Then the following must be true:
	\begin{itemize}
		\item ($4^{\text{th}}$ degree constraint)
		\[2\mu\leq n^2(r+n^2)\]
