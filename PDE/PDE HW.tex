...
\begin{enumerate}[(a)]
	...
	\item Using characteristic equations we find that 
	\[\dot{t}(s) \qquad \dot{\textbf{x}}(s)= b\qquad \dot{z}(s) = -\text{div}(b)z \]
	with initial conditions
	\[t(0) = 0\qquad \textbf{x}(0) = a\qquad z(0) = g(a).\]
	Then $\textbf{x}(s) = bs+a$. We wish for $\textbf{x}(t) = x$ so $a=x-bt$. Then, we know $\textbf{x}(s)=\textbf{x}(s,x,t)$ solves the ODE
	\[\begin{aligned}
	\dot{\textbf{x}} &= b\\
	\textbf{x}(t) &= x
	\end{aligned}\]
	Then $\frac{d}{ds}J(s,x,t) = \text{div}(b)J$ and further we may use chain rule to show that $\frac{d}{ds}J(-s,x,t) = -\text{div}(b)J(-s,x,t)$. Thus $z=g(a)J(-t,x,0)$ must solve the remaining differential equation.
	???
\end{enumerate}
