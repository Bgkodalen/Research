...
\begin{enumerate}[(a)]
	...
	\item Following the hint, consider that
	\[\begin{aligned}
	\frac{\partial}{\partial s}(u(x,t)J) &= t_su_tJ + \sum_i u_{x^i}x^i_sJ + uJ_s\\
	&= t_su_tJ + \sum_i u_{x^i}b^iJ + u\text{div}(b)J\\
	&= \left(t_su_t + \text{div}(ub)\right)J.\\
	\end{aligned}\]
	Therefore, as long as $t_s=1$, we have that $\frac{\partial}{\partial s}(u(x,t)J) = (u_t+\text{div}(ub))J = 0$. Therefore let $t=s+t_0$ and we have $u(x,s)J(s,x,s) = u(x,0)J(0,x,0) = g(x(0,x,0))$.
\end{enumerate}
