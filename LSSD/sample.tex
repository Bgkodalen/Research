...
order to remove unused entries, e.g.
...
Another useful command is the field \verb|\arXiv|: you can see it in action in reference \cite{Per02}. The entry, which is ``only'' an \texttt{arXiv} submission, was typeset (in the \texttt{.bib} file, as usual!) as
\begin{verbatim}
@Unpublished{Per02,
...
\end{verbatim}
(you can check it by yourself in the \texttt{sample-ac.bib} file) so
that you see what \verb|\arXiv| does. On a one hand, it prepends
\texttt{arXiv:} to the reference number (which was the only typeset
argument) and, on the other, it creates a clickable link. It is
important, in unpublished works, that this piece of information is in
the \verb|\note{}| field.

Finally, there is a trend in electronic publishing to endow papers
with a unique ID rather than conitnuous page numbering to precisely
locate papers within a journal. This is handled in amsplain-ac through
the field \verb|eid|, which you can complete with an optionnal
\verb|pagetotal| field. The field \verb|pages| should not be used for
these papers. See entry \verb|infrank| in \texttt{sample-ac.bib} and
\cite{infrank}.
