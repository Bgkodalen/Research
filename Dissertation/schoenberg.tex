...
In this chapter we will use polynomial rings and quotient rings, thus we will finish this section with a brief review of the topic, referring to \cite{Gallian} for a more complete introduction. Let $m$ be a fixed positive integer and define $\scR:=\bbR[x_1,\dots,x_m]$. A \emph{monomial} $t\in\scR$ is a polynomial of the form $t = \alpha\prod_{i=1}^{m}x_i^{d_i}$ for $\alpha\in \bbR$ and $d_i\in \bbZ$. For a given monomial $t=\alpha\prod_{i=1}^{m}x_i^{d_i}$, the \emph{degree} and \emph{derivative with respect to $x_j$} of $t$ is:
\[\begin{aligned}
\text{deg}(t) &= \sum_i d_i\\
\frac{d}{dx_j}(t) &= \alpha d_jx_j^{d_j-1}\prod_{i\neq j}x_i^{d_i}.
\end{aligned}\]
Any $f\in \scR$ may be written as a linear combination of monomials so that $f = \sum_i\alpha_it_i$ for $\alpha_i\in \bbR$ and monomials $t_i$. We say a monomial $t_i$ is in $f$ if

, we say $f$ is \emph{homogeneous} if there exists some constant $d$ such that $\text{deg}(t)=d$ for every monomial in $f$. A polynomial $f$ is \emph{harmonic} if $\Delta f = \sum_i \frac{\partial}{\partial x_i}\left(\frac{\partial}{\partial x_i}\left(f\right)\right) = 0$ where derivatives of $f$ are defined by extending the definition for monomials linearly.\par


Then any polynomial $f\in \scR$ is comprised of \emph{monomials} of the form $a = \prod_{i=1}^{m}x_i^{d_i}$ with $d_i\in\bbZ$ for each $1\leq i\leq m$. Given a monomial $a$, the \emph{degree} of $a$ equals the sum of the $d_i$, denoted $\text{deg}(a) = \sum_i d_i$.
