...
\end{cor}
We note that, Theorem \ref{kitchensink} \emph{(i')} tells us that $q^j_{i0} = \delta_{i,j}$ for $0\leq i\leq d$ and thus equation $\eqref{ELGeg}$ gives us that the first column of $Q^{m_i}_\ell\left(\frac{1}{m_i}L_i^*\right)$ is \[\frac{1}{\vert X\vert}\sum_j\theta_{\ell j}\vec{e}_j=\left[\begin{array}{c}
