...
Thus, using $f = 1-\sum_{i}x_i^2$, we say a polynomial $h\in\scR$ is \emph{harmonic on the sphere}\index{polynomial!harmonic!on the sphere} if there exists a harmonic polynomial $g\in[h]_f$; we similarly define \emph{homogeneous on the sphere}\index{polnomial!homogeneous!on the sphere}. Finally, we say a polynomial $h\in\scR$ is \emph{zonal}\index{polynomial!zonal} if there exists a vector $a\in \bbR^m$ and a single-variable polynomial $p(t)\in\bbR[t]$ such that $h(x) = p(\left<a,x\right>)$ for all $x\in S^{m-1}$. Note that since $g\in \left(1-\sum_ix_i^2\right)$ implies $g(x) = 0$ for all $x\in S^{m-1}$, this requirement is independent of the representative chosen from the equivalence class.\par
...
\begin{proof}
	...
	\frac{1}{\sqrt{m_i}}\vec{e}_{i}\\\hdashline[2pt/2pt]
	\vec{e}_{0}
	\end{array}\right]$. Consider that the sign of $\left[\vec{y}_\ell\right]_i$ is negative if and only if there is a negative entry in either $\vec{c}_\ell$ or $\vec{c}_{\ell-1}$ as defined in Equation \eqref{crec} and thus if and only if one of $Q_{\ell}^{m_i}\left(\frac{1}{m_i}L_i^*\right)$ or $Q_{\ell-1}^{m_i}\left(\frac{1}{m_i}L_i^*\right)$ contains a negative value. As before, define subspaces $B_j = \text{span}\left(\left[\begin{array}{c}
	\vec{p}_j\\\hdashline[2pt/2pt]
	\vec{0}
	\end{array}\right],\left[\begin{array}{c}
	\vec{0}\\\hdashline[2pt/2pt]
	\vec{p}_j
	\end{array}\right]\right)$ where $\vec{p}_j$ is the $j^\text{th}$ column of $\sqrt{\Delta_m}P$. Lemma \ref{normbound} then tell us that
	\[\text{proj}_{B_j}(\vec{y}_1) = \frac{1}{\vert X\vert}\left[\begin{array}{c}
	...
	\end{aligned}\]
	Therefore, as long as $\sum_{j=1}^d v_j\lambda_j^{2\ell^*}(1+\lambda_j^2)\leq\frac{1}{\vert X\vert}$, we guarantee $y_{\ell^*+1}$ has no negative entry. Additionally, since the bottom half of $y_{\ell^*+1}$ equals the top half of $y_{\ell^*}$, we may assume that any negative entry of $y_{\ell^*}$ appears in the bottom half, implying that $y_{\ell^*-1}$ also has a negative entry. Thus checking the first $\ell^*-1$ degrees is sufficient.
\end{proof}
...
Here, $\theta_{31}\geq0$ is not implied by FC1. Thus we have the additional constraint
...
\begin{proof}
	...
	Constraint & \emph{(i)} & \emph{(ii)} & \emph{(iii)} & \emph{(iv)} & \emph{(v)} & \emph{(vi)} & \emph{(vii)}\\\hline
	Restriction & $\theta_{31}\geq 0$ & $\theta_{42}\geq 0$ & $\theta_{51}\geq 0$&$\theta_{53}\geq 0$&$\theta_{60}\geq 0$	&$\theta_{50}\geq 0$ &$\theta_{52}\geq 0$\\	
	\end{tabular}\]
\end{proof}
Dr. Williford maintains a list of small ($\vert X\vert<10000$) feasible parameter sets for cometric schemes including primitive cometric schemes with 3 classes \cite{Willifordtable3} and $Q$-bipartite schemes with 4 classes \cite{Willifordtable4}. Using these lists, we find nine $3$-class primitive cometric schemes which are ruled out by Theorem \ref{cometricbnds} \emph{(vi)} and 11 $4$-class $Q$-bipartite schemes which are ruled out by Theorem \ref{cometricbnds} \emph{(v)}. They are as follows, listed as tuples of the form $\left(\vert X\vert,m_1\right)$:
\begin{itemize}
...
\begin{thm}\label{Qbipdegbnd}
	Suppose we have a feasible parameter set for a $Q$-bipartite association scheme with eigenmatrices $P$ and $Q$ with relations ordered naturally. Define $\lambda_j:=\frac{Q_{j,1}}{m_1}$ and $k_j = P_{0,j}$ for $0\leq j\leq d$. Define $\gamma_{\ell,j} = \begin{cases}
