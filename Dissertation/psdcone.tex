...
\end{thm}
In this section, we refer a system of lines as \emph{optimal}\index{equiangular lines!optimal} if it achieves the relative bound for the given dimension and innerproducts and \emph{near optimal}\index{equiangular lines!near optimal} if it is within one line of being optimal. We now examine two 3-class primitive association schemes to illustrate how we may build equiangular lines from these schemes. The first comes from the Halved 7-cube while the second corresponds to the Dual Polar space $B_3(2)$.
\begin{example}
...
We say a polynomial $h\in\scR$ is \emph{harmonic on the sphere}\index{polynomial!harmonic!on the sphere} if there exists a harmonic polynomial $g\in[h]$; we similarly define \emph{homogeneous on the sphere}\index{polnomial!homogeneous!on the sphere}. Finally, we say a polynomial $f\in\scR$ is \emph{zonal}\index{polynomial!zonal} if there exists a vector $a\in \bbR^m$ and a single-variable polynomial $p(t)\in\bbR[t]$ such that $f(x) = p(\left<a,x\right>)$ for all $x\in S^{m-1}$. Note that since $g\in \left(1-\sum_ix_i^2\right)$ implies $g(x) = 0$ for all $x\in S^{m-1}$, this requirement is independent of the representative chosen from the equivalence class.\par
...
Let $m$ be a fixed positive integer and let $X\subset S^{m-1}$ be a finite set of unit vectors. Let $G_X$ denote the Gram matrix of $X$, then $G_X$ is positive semidefinite ($G_X\succeq 0$). A function $f:[-1,1]\rightarrow\mathbb{R}$ is \textit{positive definite}\index{positive definite function} if, for every such finite subset $X$, $f$ applied elementwise to $G_X$ results in a positive semidefinite matrix, i.e.\ $f\circ(G_X)\succeq 0$. Here we present a Sch\"{o}nberg's Theorem \cite{Schoenberg1942} as it applies to polynomials.
