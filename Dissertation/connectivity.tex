...
Recall that a \emph{commutative association scheme} is a more general combinatorial object in which we replace property \emph{(iii)} with the requirement: For each $0\leq i\leq d$, there exists some index $i^\prime$ such that $R_i^T = R_{i^\prime}$; that is, $(x,y)\in R_i$ if and only if $(y,x)\in R_{i^\prime}$. We additionally add the requirement that for all $0\leq i,j,k\leq d$, $p^k_{ij}=p^k_{ji}$ --- thus we preserve the property that our Bose-Mesner algebra is commutative. Given a commutative association scheme $(X,\mathcal{R})$ we define the \emph{symmetrization} of the relation $R_i$ as the relation $R_i\cup R_{i'}$ noting that this is exactly $R_i$ if $i=i'$ (i.e.\ $R_i$ is already symmetric). In this way, we define the \emph{symmetrization} of $(X,\mathcal{R})$ to be $(X,\mathcal{R}')$ where
