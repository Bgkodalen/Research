...
\end{equation}
we call these constants the \emph{Krein parameters} of our association scheme. Thus we find an analogous lemma as
\begin{lem}\label{Elaw}
	Let $E_0,\dots,E_d$ be the idempotent basis matrices of a $d$-class symmetric association scheme. Then,
	\begin{enumerate}[label=$(\roman*)$]
		\item $E_0 = \frac{1}{\vert X\vert} J$;
		\item $\sum_j E_j = I$;
		\item for all $0\leq j\leq d$, $E_i^T = E_i$;
		\item for all $0\leq i,j\leq d$, $E_iE_j = \frac{1}{\vert X\vert}\sum q_{ij}^k E_k$.
	\end{enumerate}
\end{lem}

For convenience, we will organize the intersection numbers and Krein parameters into arrays $L_i$ and $L_i^*$ so that
\begin{equation}
...
\begin{thm}\label{cometricequiv}
	...
	\end{itemize}
\end{thm}
From Theorem \ref{cometricequiv} $(ii)$ we see that $E_1$ generates all of $\BMA$ using entrywise products. Paired with the fact that $\BMA$ is $d+1$ dimensional, this implies $E_1$ has exactly $d+1$ distinct entries. Thus, given some $Q$-polynomial ordering, we may order the relations so that $Q_{01}>Q_{11}>\dots>Q_{d1}$; we call such an ordering the \emph{natural ordering of relations}. 

An association scheme is said to be \emph{imprimitive} if there exists some basis relation whose graph is disconnected. In the next theorem and henceforth, we let $I_r$ (or simply $I$) denote the $r\times r$ identity matrix and $J_r$ (or $J$) will denote the all ones matrix of order $r\times r$.

\begin{thm}[see \cite{Bannai1984}, \cite{Brouwer1989},\cite{Godsil1993}] The following are equivalent:
	\begin{itemize}
		\item[(i)] $(X,\BMA)$ is imprimitive;
		\item[(ii)] for some $j>0$, $E_j$ has repeated columns;
		\item[(iii)] for some subset $\cI = \left\{i_0=0,i_1,\dots,i_s\right\}$ of $\left\{0,1,\dots,d\right\}$ and some ordering of the vertices $\sum_{h=0}^sA_{i_h} = I_w\otimes J_r$ for some integers $w$ and $r$ with $v=wr$, $1<w,r<v$;
		\item[(iv)] for some subset $\cJ = \left\{j_0=0,j_1,\dots,j_s\right\}$ of $\left\{0,1,\dots,d\right\}$ and some ordering of the vertices $\sum_{h=0}^sE_{j_h} = \frac{1}{r}\left(I_w\otimes J_r\right)$ for some integers $w$ and $r$ with $v=wr$, $1<w,r<v$;
	\end{itemize}
\end{thm}

Property $(iii)$ of the above theorem tells us that the union of relations $\bigcup_{i\in\cI}R_i$ is an equiavalence relation. Given an imprimitive association scheme, we may define a quotient scheme $(\tilde{X},\tilde{R})$ where $\tilde{X}$ is the set of equivalence classes induced and for $\tilde{x},\tilde{y}\in\tilde{X}$, $(\tilde{x},\tilde{y})\in\tilde{R}_i$ if there exists points $x\in \tilde{x}$ and $y\in\tilde{y}$ such that $(x,y)\in R_i$. The following lemma relates the second eigenmatrices of an imprimitive association scheme and its quotient scheme.
\begin{lem}[\cite{Brouwer2003},\cite{Martin2007}]\label{repeatedcols}
	Let $P$ and $Q$ be the first and second eigenmatrix of an imprimitive association scheme with a system of imprimitivity given by index sets $\cI$ and $\cJ$. Let $\tilde{Q}$ be the second eigenmatrix of the quotient scheme. Then for any $j\in \cJ$ and $h\in\tilde{h}$ we have
	\[\tilde{Q}_{\tilde{h}j} = Q_{hj}.\]
\end{lem}

We say a cometric association scheme is $Q$-bipartite if the Krein parameters satisfy $q^k_{ij} = 0$ whenever $i+j+k\notin2\bbZ$; we say a cometric association scheme is $Q$-antipodal if the Krein parameters satisfy $q^k_{dd}=0$ whenever $k\notin\left\{0,d\right\}$. The following theorem of Suzuki characterizes the imprimitive cometric association schemes.
...
\end{lem}
The association scheme structure allows us to improve on the naive upper bound given in Corollary \ref{regnaive}.
\begin{thm}
...
\begin{proof}
	...
	1 & -m & f & m-v & g\\
	\end{array}\right]\]
	Since we now have the entire $Q$ matrix, we may standard techniques (see \cite[Theorem 2.3.2]{Brouwer1989}) to find the Krein parameters of our scheme. In particular, $q^3_{11} = q^4_{12} = 0$ as well as
