...
This proposition tells us that for us to have any hope of building an association scheme from these relations, we must consider graphs which are not only regular, but strongly regular. Note that the converse of Proposition \ref{projnec} is certainly not true; the first projective double of Example \ref{doublecoverc4} does not result in an association scheme even though $C_4$ is strongly regular. Thus we will look for further necessary conditions for the projective double to result in an association scheme. Before we continue with this investigation, we review a few details of strongly regular graphs which will be useful for us.
...
\end{proof}
Note that this construction does not result in a 4-class association scheme. We see this by noting that all off diagonal entries in $G$ must be positive. Thus we may split our projective design into two sets $L^+$ and $L^-$ where $L^+$ contains all the columns of $U$ while $L^-$ contains their negatives. Then for vectors $u\perp v$, the number of vectors $w$ such that $\left<v,w\right> = \left<u,w\right>=\alpha$ could be either $0$ (if $v\in L^+$ and $u\in L^-$) or $\lambda$ (if $v,w\in L^+$). Thus this value is not solely dependent on the inner product $\left<u,v\right>$ and thus $p^2_{11}$ is not well defined. While this does not solve our question of which projective doubles are association schemes, it does provide us with a better upper bound for strongly regular graphs than we saw in Corollary \ref{regnaive}. For example, this gives a projective double of the Petersen graph in dimension $5$ while Corollary \ref{regnaive} produces one in dimension $9$. Now consider the following theorem of Delsarte.
...
\end{thm}
We will refer to a \emph{Delsarte coclique} as a coclique for which this bound is tight. This theorem, along with Proposition \ref{cocliquebnd}, gives a lower bound on the dimension of any projective double in terms of the spectrum. The following corollary shows what must happen if the bound is tight for a projective double of some strongly regular graph containing a Delsarte coclique.
\begin{cor}\label{snegsquare}
	Let $\Gamma$ be a nonempty strongly regular graph with $v$ vertices, valency $k$ and smallest eigenvalue $s$. Assume there exists a Delsarte coclique $C$ in $\Gamma$. Then any projective double in $\mathbb{R}^{\vert C\vert}$ with inner product $\alpha$ exists only if $s = -\alpha^{-2}$. Further, unless $\Gamma$ is complete bipartite, we must have $\alpha^{-1}\in \mathbb{Z}$.
\end{cor}
\begin{proof}
...
\end{proof}
While this theorem does not give us information about projective doubles of strongly regular graphs without Delsarte cocliques, there are many common examples which contain these cocliques for which we may apply our theorem to. For instance consider the following result.
\begin{cor}
...
\end{proof}
It is worth pointing out that the points of the dodecahedron are often considered a projective double of the Peterson graph, yet these points are realizable in $\mathbb{R}^3$. In fact, the dodecahedron is a 4-distance antipodal set of size 20 with a mapping from these antipodal points to the vertices of the Petersen graph where the angle between two lines depends only on whether or not their images are adjacent. The problem with this construction is that orthogonal lines map to adjacent points in the Petersen graph, rather than non-adjacent points. Thus we instead say the dodecahedron is a projective double of the complement of the Petersen graph. This complement has valency 6 and smallest eigenvalue -2, requiring a Delsarte coclique to have size $2$. Thus Corollary \ref{snegsquare} requires nothing of a projective double in $\mathbb{R}^3$.

We finish this section with a conjecture concerning necessary and sufficient conditions for a projective double to result in an association scheme.
...
\begin{thm}
	Let $\Gamma$ be a strongly regular graph with $v$ vertices, and spectrum $k^1,r^f,s^g$. Let $L$ be a projective double of $\Gamma$ in $\mathbb{R}^m$ with inner product $\alpha$ and $m<v$. Then the Gram matrix of $L$ generates a Bose-Mesner algebra $\BMA$ only if $m = v\left(1+k\alpha^{2}\right)^{-1}$. Further, if $m(m+1)< 2v$ then $\BMA$ is the Bose-Mesner algebra of a $4$-class $Q$-bipartite association scheme and $s = -\alpha^{-2}$.
\end{thm}
\begin{proof}
	We prove this by building the $Q$ matrix of the resultant scheme. First let $\BMA$ and $\BMB$ be the Bose-Mesner algebras of scheme generated by $G$ and the scheme given by $\Gamma$ respectively. Recall that the relations of $\BMA$ must correspond to the distinct values in $G$, thus define $A_0,A_1,A_2,A_3,$ and $A_4$ corresponding to the values $1,\alpha,0,-\alpha,$ and $-1$. By definition of the projective double, find that $R_0\cup R_4$ gives a system of imprimitivity where $\BMB$ is the quotient of $\BMA$ using this system. Thus there must exist three basis idempotents of $\BMA$, call them $E_0$, $E_2$, and $E_4$ which are span a Schur-closed algebra isomorphic to $\BMB$. Equation \eqref{PQsrg} tells us that the second eigenmatrix of $\BMB$ is
	\[\tilde{Q} = \left[\begin{array}{ccc}
	1 & f & g\\
	1 & \frac{fr}{k} & \frac{gs}{k}\\
	1 & \frac{f(1+r)}{k+1-v} & \frac{g(1+s)}{k+1-v}
	\end{array}\right].\]
	Since our quotient map $\tilde{\psi}:\text{span}\left\{E_0,E_2,E_4\right\}\rightarrow \BMB$ preserves Schur multiplication we must have, for example, 
	\[\frac{Q_{12}}{\vert X\vert}\phi(E_2) =\phi(E_2\circ A_1) = \phi(E_2)\circ\phi(A_1) = \phi(E_2)\circ\phi(A_3) =\phi(E_2\circ A_3) = \frac{Q_{32}}{\vert X\vert}\phi(E_2) \]
	and thus $Q_{12} = Q_{32}$. Similar computations give both $Q_{1i} = Q_{3i}$ and $Q_{0i} = Q_{4i}$ for $i\in\left\{0,2,4\right\}$ meaning that the second eigenmatrix of $\BMA$ must have the form
	...
	1 & * & f & * & g\\
	\end{array}\right]\]
	where the $*$ denotes some unknown value. Let $n_1$ and $n_3$ be the remaining two multiplicities corresponding to $E_1$ and $E_3$ respectively. Note that since $1+f+g=v$, we must have $n_1+n_3 = v$ as well. Now, by construction, $G = A_0 + \alpha A_1 -\alpha A_3 -A_4$ and thus $G\in \BMA$. However, the top left entry of $GE_2$ is given by
	\[\left[GE_2\right]_{11} = f+k\frac{fr}{k}\alpha - k\frac{fr}{k}\alpha -f = 0.\]
	Similarly, the top left entries of both $GE_4$ and $GE_0$ are also $0$. This implies that $GE_i = 0$ for $i\in\left\{0,2,4\right\}$. Thus there exists constants $c_1$, and $c_3$ such that $G = c_1E_1+c_3E_3$. However, since $m<v$, we must have either $c_1$ or $c_3$ to be 0; without loss of generality assume $c_3 =0$. This gives us that $G = \frac{\vert X\vert}{m}E_1$, implying that $G$ has only two eigenvalues $\frac{\vert X\vert}{m}$ and $0$. Then \cite{Delsarte1977} tells us that $L$ must be a 3-design and further, using \cite[Theorem 5.5]{Delsarte1977}, we find that for any $x\in L$, $\sum_{y\in L} Q_2^m\left(\left<x,y\right>\right) = 0$ where $Q_2^m(t)$ is the degree 2 Gegenbauer (see Section \ref{gegdef}) polynomial. Thus $\frac{m(2+2k\alpha^2)-2v}{m-1} = 0$, giving
	\[m = v(1+k\alpha^2)^{-1}.\]
	
	Now, we may return to our $Q$ matrix and fill in the entries of the first column. Further, the orthogonality relations (Lemma \ref{orthorels}) tell us that the row sum of $Q$ must be 0 (except for the first row). Using the same fact for $\tilde{Q}$, we may find the final column as well. 
	...
	1 & -m & f & m-v & g\\
	\end{array}\right]\]
	From here, we use Lemma $\ref{kitchensink}$ $(xiii^\prime)$ to show that $q^3_{11} = q^4_{12} = 0$. Further, 
	...
	\end{aligned}\]
	Noting that $v- m(1+\alpha^2r)>v-m(1+\alpha^2k) = 0$ as long as $r\neq k$ ($\Gamma$ is not complete) we have three Krein parameters nonzero. Thus the only possible obstruction is $q^4_{11}$. Calculating this similarly, we find
	\[q^4_{11} = \frac{1}{2v g}\sum_{h=0}^d\left(k_hQ_{h1}Q_{h1}Q_{h4}\right) = \frac{m^2g\left(1+\alpha^2s\right)}{v}.\]
	Thus $q^4_{11}=0$ if and only if $s = -\alpha^{-2}$. However if $\frac{m(m+1)}{2}< 1+f+g = v$ then the absolute bound tells us that $q^4_{11} = 0$, giving our result.
\end{proof}
