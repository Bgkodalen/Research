...
Let $\Gamma$ be an undirected graph on $v$ vertices. A \emph{projective double} of $\Gamma$ is an antipodal (spherical) $4$-distance set, say $L = \left\{\pm \ell_1,\dots,\pm \ell_v\right\}$, such that there exists a mapping $\phi:L\rightarrow V\Gamma$ fulfilling the following properties:
\begin{enumerate}[label=(\roman*)]
	\item for all $i$, $\phi(\ell_i) = \phi(-\ell_i)$,
	\item for $i\neq j$, $\phi(\ell_i) \sim \phi(\ell_j)$ implies $\ell_i\not\perp\ell_j$,
	\item for $i\neq j$, $\phi(\ell_i) \not\sim \phi(\ell_j)$ implies $\ell_i\perp\ell_j$.
\end{enumerate}
Note that since $L$ is antipodal, the five possible inner products (including $1$) must be symmetric about $0$. That is, $\alpha$ is a possible distance if and only if $-\alpha$ is as well. Therefore the five possible distances must be $1>\alpha>0>-\alpha>-1$ for some $\alpha\in(0,1)$. Thus $\ell_i\not\perp \ell_j$ for $i\neq j$ implies $\left<\ell_i,\ell_j\right> = \pm\alpha$.

We are initially interested in when such a projective double may exist, however this ends up being uninteresting if we allow any angle and any dimension. Consider the following proposition
\begin{prop}\label{dirnaive}
	For any simple graph $\Gamma$, there exists a projective double in $\mathbb{R}^{m}$ for some $m\leq \vert V\Gamma\vert$.
\end{prop}
\begin{proof}
	Let $\Gamma = (V,E)$ be given. Now orient every edge of $\Gamma$ and define $e_i^+$ and $e_i^-$ so that $e_i = (e_i^-,e_i^+)$. Let $M$ be the matrix with rows indexed by vertices and columns indexed by edges such that
	\[\left[M\right]_{ij} =\begin{cases} 1 &\text{ if }v_j = e_i^+\\
	-1 &\text{ if }v_j = e_i^-\\
	0 &\text{ otherwise. }
	\end{cases}\]
	Then we find 
	...
	0 & \text{ otherwise}
	\end{cases}\]
	where $k_i$ is the degree of vertex $i$. We therefore have correct interaction between distinct vectors, however unless $k_i$ is constant independent of $i$, these are not unit vectors. To solve this, let $k = \max_{i}(k_i)$ (the max degree of $\Gamma$) and define the diagonal matrix $D$ whose $i^\text{th}$ diagonal entry is $\sqrt{k-k_i}$. Then the matrix $N = \left[\begin{array}{c|c}
	M & D
	...
	0 & \text{ otherwise.}
	\end{cases}\]
	Thus the rows of $N$ along with their negatives result in a \emph{projective double} of $\Gamma$. Further the rank of $N$ is no larger than $\max\left\{\vert V\vert, \vert V\vert+\vert E\vert\right\} = \vert V\vert$.
\end{proof}
\begin{cor}\label{regnaive}
	For any regular graph $\Gamma$ with at least one edge, there exists a projective double in $\mathbb{R}^m$ for some $m\leq T$ where $T$ is the number of edges in any spanning forest.
\end{cor}
\begin{proof}
	We follow the same idea as with Proposition \ref{dirnaive} however we note that since our graph is regular, the rows of $M$ with their negatives suffice to serve as our vectors. In this case, $\rank(M) \leq \text{min}\left\{\vert V\vert,\vert E\vert\right\}$. However, we can improve this substantially by noting the following: Consider any cycle $C$ in our graph and assume without loss of generality that $C = \left\{e_1,\dots,e_s\right\}$. Then the the corresponding columns of $M$ are linearly dependent, that is $M_{e_s} = \sum_{i=1}^{s-1}M_{e_{i}}$. Then reorder the columns of $M$ so that the first $T$ edges correspond to the edges of a spanning forest. Then including any column after these $T$ columns results in a linearly dependent set of columns, forcing $\text{rank}(M) \leq \text{min}\left\{\vert V\vert,T\right\}$. Further, since our graph cannot have any isolated vertices, we know $\vert T\vert\leq \vert V\vert$ and therefore $\text{rank}(M) \leq T$
\end{proof}
...
\end{prop}
\begin{proof}
	Assume we have a projective double of $\Gamma$ with $\phi(\ell_i) = i$ for each $1\leq i\leq\vert V\vert$. Let $\alpha = \alpha\left(\Gamma\right)$ and $S = \left\{i_1,\dots,i_\alpha\right\}$ be an independent set of maximal size. Then $\ell_i\perp\ell_j$ for each $i,j\in S$ with $i\neq j$. Thus $\left\{\ell_i,\dots,\ell_{i_\alpha}\right\}$ form a set of orthogonal vectors, forcing the dimension to be at least $\alpha$.
\end{proof}
We therefore are interested in when we may achieve this lower bound. For example, consider the graph $C_4$. This is a regular graph and thus we may apply Corollary \ref{regnaive} to find that there exists a projective double in $\mathbb{R}^3$. In fact, the columns of the following matrix serve as one such projective double. 
...
\end{array}\right]\]
However, the maximum independent set in this graph has size $2$. Thus Proposition \ref{cocliquebnd} allows for the possibility of a projective double in $\mathbb{R}^2$. In fact, the columns of the following matrix give us exactly that.
...
\end{array}\right]\]
Note that the rank of $U_1 = 3$, thus $U_1$ and $U_2$ are non-isomorphic representations. It is important to note here that, as one might expect, there are many graphs for which we cannot find a projective double in the dimension given by the independence number. In other words, Proposition \ref{cocliquebnd} if often not tight. To see an example, consider the following proposition

\begin{prop}
	Let $\Gamma$ be a complete multipartite graph with $w$ parts of size $v$. Let $U$ be the matrix with columns corresponding to a projective double of $\Gamma$ where $\text{rank}\left(U\right) = \alpha\left(\Gamma\right) = v$. Then the columns of $U$ form a set of $w$ mutually unbiased bases in $\mathbb{R}^v$.\qed
\end{prop}
\begin{proof}
	Follow the same proof as in Proposition \ref{cocliquebnd}, noting that non orthogonal vectors must have a fixed inner product.
\end{proof}
\begin{cor}
	Let $\Gamma = K_{2\times t}$ for $t\neq 0\mod 4$. Then $\Gamma$ does not have a projective double in $\mathbb{R}^\alpha$ for $\alpha = \alpha\left(\Gamma\right)$.
\end{cor}
\begin{proof}
...
Both represent projective doubles of the 4-cycle however. In the case of $U_1$, when $\alpha = \frac{1}{\sqrt{2}}$, $G_{\alpha}$ gives the 8-cycle and $\left\{G_1,G_\alpha,G_0,G_{-\alpha},G_{-1}\right\}$ is a 4-class association scheme. However in the case of $U_2$, when $\alpha = \frac{1}{2}$, $G_{\alpha}$ is $C_4\cup C_4$ and $\left\{G_1,G_\alpha,G_0,G_{-\alpha},G_{-1}\right\}$ does not give us an association scheme.
\begin{thm}
...
\section{Strongly regular graphs--a review}
