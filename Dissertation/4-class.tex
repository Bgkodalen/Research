...
\end{tikzpicture}\]
Note the dashed lines in $\Gamma_{\frac{1}{\sqrt{2}}}$ appear only to emphasize the extra symmetry. Thus $U_1$ and $U_2$ are clearly different as their corresponding graphs are non-isomorphic double covers of $C_4$. We similarly define the five relations $R_0,\dots,R_4$ on any $OPD_m(\Gamma;\beta)$ via the inner products $1$, $\beta$, $0$, $-\beta$, and $-1$ respectively so that, for instance, $(x,y)\in R_2$ if and only if the corresponding vectors are orthogonal. These five relations satisfy conditions $(i)$--$(iii)$ of an association scheme, leaving condition $(iv)$ as our final test; that is, we must determine if the intersection numbers are well-defined. In the case of the first $OPD$, we find that even though both $(v_1,v_5)$ and $(v_1,v_6)$ are in $R_2$,
...
\begin{thm}\cite[Theorem 3.8]{Delsarte1973} \label{delsarte}
	...
	A coclique $C\subset V$ achieves equality in this bound if and only if every vertex $\beta\notin C$ has the same number of neighbors (namely $-s$) in $C$.\qed
\end{thm}
...
\begin{proof}
	...
	Thus we must have $m\left(2+2k\beta^2\right)\geq2v$ with equality if and only if our OPD admits a spherical $2$-design. Seen later in \cite{Delsarte1977}, we find that this occurs exactly when $G$ has two distinct eigenvalues: $0$ and $\frac{2v}{m}$; that is, $\frac{m}{2v}G$ is idempotent.
\end{proof}
...
\begin{proof}
	...
	\end{array}\right].\]
	We note that the definition of $E_0$ implies an analogous structure using $\tilde{E}_0$. Further, these constructions imply that
	...
	\end{cases}\]
	It follows that $E_0$, $E_2$, and $E_4$ are pairwise orthogonal idempotents. Now, Lemma \ref{mindim} tells us that $E_1$ is also idempotent since $m = v\left(1+k\beta^2\right)^{-1}$. Additionally, the block structure of the matrices $E_0$, $E_1$, $E_2$, and $E_4$ imply that $E_1E_0=E_1E_2=E_1E_4 = 0$ telling us that $E_0$, $E_1$, $E_2$, $E_4$ are pairwise orthogonal idempotents. In view of Lemma \ref{AElem} $(iii')$, we define $E_3 = I-E_0-E_1-E_2-E_4$ and may immediately compute.
	\[E_3^2 = \left(I-E_0-E_1-E_2-E_4\right)^2 = I-E_0-E_1-E_2-E_4\]
	and $E_3E_i = 0$ for $i\neq 3$. Therefore the vector space spanned by $\left\{E_0,E_1,E_2,E_3,E_4\right\}$ is symmetric, closed under matrix multiplication, and contains both the identity matrix and the all ones matrix. In order to show this algebra is a Bose-Mesner algebra, we must also show it is closed under entrywise products. By construction of $E_1$, $E_2$, and $E_4$, any linear combination of $E_0,\dots,E_4$ has at most $5$ distinct entries. This gives the dimension of $\left<E_1\right>_\circ$ as five. Since we have five basis matrices, this forces $\left<E_0,\dots,E_4\right> = \left<E_1\right>_\circ$ and thus our algebra is a Bose-Mesner algebra. We finish our proof by listing the Krein parameters of our association scheme, referring to the parameter definitions in Theorem \ref{srgparams} for the strongly regular graph parameters.
...
\begin{cor}
	...
	Let $\Gamma$ be a conntected strongly regular graph with $v$ vertices and eigenvalues $k>r>s$ which contains a Delsarte coclique. Let $G$ be the Gram matrix of an $OPD_m(\Gamma;\beta)$ with $m = v\left(1-\frac{k}{s}\right)^{-1}$. Then $\left<G\right>_\circ$ is the Bose-Mesner algebra of a 4-class $Q$-bipartite association scheme.
\end{cor}
\begin{proof}
