...
Let $\Gamma$ be an undirected graph on $v$ vertices. A \emph{projective double} of $\Gamma$ is an antipodal (spherical) $4$-distance set, say $L = \left\{\pm \ell_1,\dots,\pm \ell_v\right\}$ with inner products $A = \left\{\pm\left<\ell_i,\ell_j\right>\right\}$, such that there exists a mapping $\phi:L\rightarrow V\Gamma$ fulfilling the following properties:
...
\end{enumerate}
Note that since $L$ is antipodal, the five possible inner products (including $1$) must be symmetric about $0$; that is, $\alpha\in A$ if and only if $-\alpha\in A$. Therefore the five possible distances must be $1>\alpha>0>-\alpha>-1$ for some $\alpha\in(0,1)$. Thus $\ell_i\not\perp \ell_j$ for $i\neq j$ implies $\left<\ell_i,\ell_j\right> = \pm\alpha$.

We are initially interested in when such a projective double may exist, however this ends up being uninteresting if we allow our dimension to be large enough.
...
\begin{proof}
	...
	0 & \text{ otherwise.}
	\end{cases}\]
	Thus the rows of $N$, along with their negatives, result in a projective double of $\Gamma$. Further the rank of $N$ is no larger than $\max\left\{\vert V\vert, \vert V\vert+\vert E\vert\right\} = \vert V\vert$.
\end{proof}
...
\end{proof}
We are interested in when we may achieve this lower bound. For example, consider the graph $C_4$. This is a regular graph and thus we may apply Corollary \ref{regnaive} to find that there exists a projective double in $\mathbb{R}^3$. In fact, the columns of $U_1$ serve as one such projective double with $\alpha = \frac{1}{2}$. 
...
\end{array}\right]\]
However, the maximum independent set in this graph has size $2$. Thus Proposition \ref{cocliquebnd} allows for the possibility of a projective double in $\mathbb{R}^2$. In fact, the columns of $U_2$ give us exactly that with $\alpha = \frac{\sqrt{2}}{2}$.
...
\end{array}\right]\]
To see the difference between $U_1$ and $U_2$, consider the graph $\Gamma_\alpha(L,E)$ where $(\ell_i,\ell_j)\in E$ if and only if $\left<\ell_i,\ell_j\right> = \alpha$. Then using the columns of $U_1$ as our projective double, we find that $\Gamma_{\frac{1}{2}}$ is given below
\[\begin{tikzpicture}[shorten >=1pt,auto,node distance=2cm,
thin,main node/.style = {circle,draw, inner sep = 0pt, minimum size = 15pt}]

\node[main node,fill=white] (1) {1};
\node[main node,fill=white] [right of = 1](2) {3};
\node[main node,fill=white] [above of = 1](3) {7};
\node[main node,fill=white] [above of =2](4) {5};
\node[main node,fill=white] [right of =2](5) {2};
\node[main node,fill=white] [right of = 5](6) {4};
\node[main node,fill=white] [above of = 5](7) {8};
\node[main node,fill=white] [above of =6](8) {6};
...
edge node {} (6);
\end{tikzpicture}\]
However if we instead consider $U_2$ as our projective design, then $\Gamma_{\frac{\sqrt{2}}{2}}$ is given by
\[\begin{tikzpicture}[shorten >=1pt,auto,node distance=2cm,
	thin,main node/.style = {circle,draw, inner sep = 0pt, minimum size = 15pt}]
	
	\node[main node,fill=white] (1) {1};
	\node[main node,fill=white] [right of = 1](2) {5};
	\node[main node,fill=white] [above of = 1](3) {7};
	\node[main node,fill=white] [above of =2](4) {4};
	
	\node[main node,fill=white] [right of =2](5) {3};
	\node[main node,fill=white] [right of = 5](6) {8};
	\node[main node,fill=white] [above of = 5](7) {6};
	\node[main node,fill=white] [above of =6](8) {2};
	...
	edge node {} (6);
\end{tikzpicture}\]
Thus $U_1$ and $U_2$ are non-isomorphic as their corresponding graphs are non-isomorphic double covers of $C_4$. We note here that the Gram matrix of $U_2$ generates a Bose-Mesner algebra, which is both $P$-polynomial and $Q$-polynomial, corresponding to the association scheme $C_8$; in a later section we will examine further when this occurs. As one might expect, there are many graphs for which we cannot find a projective double in the dimension given by the independence number. In other words, Proposition \ref{cocliquebnd} if often not tight. To see an example, consider the following proposition
